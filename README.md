...
This provider is designed to interface with **certer**, a custom, containerized certificate manager solution currently supporting Let's Encrypt and ZeroSSL. Built on top of the `lego` Go library, it has the capacity to support more ACME providers in the future. Both the [Certer control plane](https://github.com/Menschomat/certer) and this [Terraform provider](https://github.com/Menschomat/terraform-provider-certer) are open-source projects.
...
* `allowed_certificates` (List of String, Optional) - The list of certificate configuration UUIDs this standard token is authorized to fetch (ignored for admin tokens).
...
| **Fetch Key (Standard)** | `false` | List of Team UUIDs | Retrieves raw PEM certificate chain and private key assets (`/api/v1/certificates`) for certificates in `allowed_certificates` AND `team_id` in `allowed_teams`. Cannot access configuration. |
